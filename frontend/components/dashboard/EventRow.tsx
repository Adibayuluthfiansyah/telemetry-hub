...
function getPayloadPreview(event: EventEnvelope): React.ReactNode {
  if (event.event_type === "TELEMETRY_RECEIVED" && event.payload?.metrics) {
    return (
      <div className="flex gap-4 mt-1 pl-6 font-data-mono text-[11px] leading-relaxed">
        {event.payload.metrics.map((metric, idx) => (
          <div key={idx} className="flex items-center gap-1">
            <span className="text-on-surface-variant">{metric.key.toUpperCase()}</span>
            <span className={cn("font-medium", metric.unit === "celsius" || metric.unit === "fahrenheit" ? "text-primary" : metric.unit === "percent" ? "text-secondary" : "text-on-surface")}>
              ...
              <span className="text-on-surface-variant text-xs ml-0.5">{metric.unit === "celsius" ? "°C" : metric.unit === "fahrenheit" ? "°F" : metric.unit === "percent" ? "%" : metric.unit}</span>
            </span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center justify-between text-on-surface-variant">
        <span className="font-data-mono text-[11px]">{timestamp}</span>
        <span className="bg-surface-container-high px-1 font-data-mono text-[10px]">
      ...
      <div className={cn("flex items-center gap-2 font-medium", textColor)}>
        ...
        <span className="font-label-caps text-[11px]">{label}</span>
      </div>
      {getPayloadPreview(event)}
      {message && (
        <div className={cn("pl-6 text-on-surface-variant opacity-80 font-data-mono text-[11px]", textColor)}>
          {message}
        </div>
      )}
    </div>
